...
export class FullcourseComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // ✅ Get course details
    this._service.getCourseListByName(this.courseName)
      .subscribe(data => {
        this.courselist = data;

        // ✅ SET COURSE VIDEO FIRST
        if (this.courselist.length > 0) {
          const courseVideoUrl = this.courselist[0].courseid;
          this.video = this.courselist[0].youtubeurl
        }
      });


   this._service.getChappterListByCourseName(this.courseName)
    .subscribe(data => {
